...
func TestDeleteSubscriber(t *testing.T) {
	...
	mockRepo.DeleteSubscriberMock.Return(nil)
	svc := New(mockRepo)

	ctx := context.Background()
	_, err := svc.DeleteSubscriber(ctx, &api.ID{Id: int64(testId)})
...
func TestDeleteSubscriberNotFound(t *testing.T) {
	...
	mockRepo := NewRepositoryMock(mc)
	mockRepo.DeleteSubscriberMock.Return(repository.ErrNotFound)
	svc := New(mockRepo)

	ctx := context.Background()
	_, err := svc.DeleteSubscriber(ctx, &api.ID{Id: 1})
